...
with st.form("card_form"):
    ...
    upload_type = st.radio("업로드 유형 선택", ["🖼️ 이미지 슬라이드 (최대 5장)", "🎬 동영상 (비디오 배경)"], horizontal=True)
    
    img_files = None
    vid_file = None
    
    if "이미지" in upload_type:
        img_files = st.file_uploader("이미지 파일들을 선택해주세요 (PNG, JPG)", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
        ...
        vid_file = st.file_uploader("동영상 파일을 선택해주세요 (MP4, MOV)", type=["mp4", "mov", "webm"])
